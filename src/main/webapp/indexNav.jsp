...
<nav class="navbar navbar-expand-lg navbar-light py-0 position-fixed fixed-top vw-100" role="navigation"style="z-index: 1000;">
  <div class="container-fluid px-3 bg-light py-2">
    ...
    <!-- <li class=""> -->
    <c:if test="${cookie['carrent']!=null}">
      <div class="navbar-toggler-custom d-flex align-items-center justify-content-center d-lg-none">
        <div class="nav-item px-3">
          <a class="nav-link nav-link-black fs-5" href="/search" name="menu-book">Book</a>
        </div>
    ...
    <c:if test="${cookie['carrent']==null}">
      <div class="navbar-toggler-custom d-flex align-items-center justify-content-center">
        <div class="nav-item px-1 px-md-2 px-lg-3">
          <a class="nav-link fs-5 nav-link-black" href="./login">Teacher</a>
        </div>
        <div class="nav-item px-1 px-md-2 px-lg-3">
          <a class="nav-link fs-5 nav-link-black" href="./login">Student</a>
        </div>
      </div>
    </c:if>
    <c:if test="${cookie['carrent']!=null}">
      <div class="navbar-collapse floating-nav flex-grow-0 position-absolute position-lg-static" id="navbarNavDropdown">
        ...
        <ul class="navbar-nav bg-light">
          <li class="nav-item px-3 d-none d-lg-block">
            <a class="nav-link fs-5" href="/search" name="menu-book">Book</a>
          </li>
        ...
        <div class="collapse-nav-bg position-fixed d-block d-lg-none vw-100 top-0" style="z-index: -1;background-color:#4a4a4a4a;height:calc(100vh - 4.125rem)">
          <!-- cross for menu bar close -->
          <div class="position-absolute mouse-pointer navbar-toggler-cross" style="bottom:20%;left:50%">
            <div class="border-start border-secondary position-absolute top-50 start-50 border-3" style="height:2rem;transform: rotate(45deg);"></div>
            <div class="border-start border-secondary position-absolute top-50 start-50 border-3" style="height:2rem;transform: rotate(-45deg);"></div>
          </div>
        </div>
      </div>
